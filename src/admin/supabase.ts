...
const order = (row:Row):Order => ({ orderId:text(row.id), orderNumber:text(row.order_number) || text(row.id), customer:text(row.customer_name), email:text(row.customer_email), phone:text(row.customer_phone), fulfilment:'Pickup', paymentStatus:paymentStatus(row.payment_status), total:number(row.total), status:orderStatus(row.status), createdAt:text(row.created_at), items:[], itemsCount:number(row.items_count), notes:'' });
...
export async function getOrders(limit?:number) {
  let query=client().from('orders').select('id,order_number,customer_name,customer_email,customer_phone,payment_status,total,status,created_at,items_count').order('created_at',{ascending:false});
...
export async function getCustomers(){const {data,error}=await client().from('orders').select('customer_name,customer_email,customer_phone,total,created_at').order('created_at',{ascending:false});if(error)fail(error);const rows=(data??[]) as Row[];const map=new Map<string,{email:string;name:string;phone:string;orders:number;spend:number;lastOrder:string}>();for(const row of rows){const email=(text(row.customer_email)||text(row.customer_phone)||'').trim().toLowerCase()||'_';const entry=map.get(email);const name=text(row.customer_name)||'Unknown';const phone=text(row.customer_phone);const total=number(row.total);const createdAt=text(row.created_at);if(entry){entry.orders++;entry.spend+=total;if(createdAt>entry.lastOrder)entry.lastOrder=createdAt;if(!entry.name||entry.name==='Unknown')entry.name=name;if(!entry.phone)entry.phone=phone}else{map.set(email,{email:text(row.customer_email)||'',name,phone,orders:1,spend:total,lastOrder:createdAt})}}return Array.from(map.entries()).map(([email,c])=>({id:email,name:c.name,email:c.email||email,orders:c.orders,spend:c.spend,lastOrder:c.lastOrder})).sort((a,b)=>b.lastOrder.localeCompare(a.lastOrder))}
