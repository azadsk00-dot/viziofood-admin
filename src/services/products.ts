...
export interface CustomerProduct{id:string;name:string;description:string;price:number;category:ProductCategory;categoryId:string|null;active:boolean;available:boolean;featured:boolean;popular:boolean;createdAt:string;updatedAt:string;imageUrl:string|null;preparationTime:number;displayOrder:number}
...
export async function fetchActiveProducts():Promise<CustomerProduct[]>{const {data,error}=await client().from('products').select('id,name,description,price,category,category_id,active,available,featured,popular,created_at,updated_at,image_url,preparation_time,display_order').eq('active',true).eq('available',true).is('archived_at',null).order('display_order').order('name');if(error){console.error(error);throw error}return(data??[]).map(row=>({id:String(row.id),name:String(row.name),description:String(row.description??''),price:Number(row.price),category:String(row.category??'Pasta'),categoryId:row.category_id?String(row.category_id):null,active:Boolean(row.active),available:Boolean(row.available),featured:Boolean(row.featured),popular:Boolean(row.popular),createdAt:String(row.created_at),updatedAt:String(row.updated_at),imageUrl:row.image_url?String(row.image_url):null,preparationTime:Number(row.preparation_time??15),displayOrder:Number(row.display_order??0)}))}
