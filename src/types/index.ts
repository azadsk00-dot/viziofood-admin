...
export interface CartModifier {
  ...
  name: string;
  price: number;
  /** 'override' replaces the base price (sizes); 'adjustment' adds (default). */
  priceMode?: 'adjustment' | 'override';
}

/**
 * One combo (bundle) choice, e.g. group "Choose your pasta" → product
 * "Lasagna" at upgrade $0 (included). The combo product's own price is the
 * fixed bundle price; selections never add the child's standalone price —
 * only a positive upgrade price contributes.
 */
...
export interface CartItem {
  ...
  instructions: string;
  /** Present when this line is a combo: the validated group selections. */
