...
export async function getModifierOptions(): Promise<ModifierOption[]> {
  const { data, error } = await supabase
    .from('modifiers')
    .select('id,group_id,name,description,price,pricing_mode,active,display_order')
    .order('display_order')
    .order('name');
  if (error) throw new Error(error.message);
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      pricingMode: r.pricing_mode === 'override' ? 'override' : 'adjustment',
      active: Boolean(r.active),
      displayOrder: Number(r.display_order ?? 0),
    };
  });
...
export async function saveModifierOption(option: Partial<ModifierOption> & { id?: string; groupId: string }): Promise<void> {
  ...
  if (option.pricingMode !== undefined) payload.pricing_mode = option.pricingMode;
