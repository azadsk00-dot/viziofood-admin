...
export default function SettingsScreen(): React.ReactElement {
  ...
  const stepper = (label: string, key: 'warnMinutes' | 'urgentMinutes' | 'managerMinutes' | 'overdueMinutes', step: number, min: number, max: number, hint: string) => (
    ...
    <Screen scroll>
      ...
      <View style={[styles.card, { backgroundColor: theme.surface }]}>
        ...
        <Text style={[styles.hint, { color: theme.textDim }]}>
          New orders show one heads-up notification with this sound — no repeat loop, nothing to
          acknowledge. Test the volume from where the food is prepared.
        </Text>
        <View style={styles.testRow}>
          <BigButton
      ...
      <View style={[styles.card, { backgroundColor: theme.surface }]}>
        ...
        <Toggle label="Auto-prepare new orders" value={settings.autoPrepareNewOrders} onChange={(v) => update({ autoPrepareNewOrders: v })} hint="New orders move NEW → PREPARING automatically — even backgrounded or locked (fresh orders only, once each)" />
      </View>
      ...
      <View style={[styles.card, { backgroundColor: theme.surface }]}>
        <Text style={[styles.hint, { color: theme.textDim }]}>
          The tablet prints straight to the kitchen printer over Wi-Fi. Set the printer's IP address under
          Printer in the side menu — nothing else to configure.
        </Text>
      </View>
