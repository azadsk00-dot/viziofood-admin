...
describe('isAlertWorthy — duplicate + invalid-order protection', () => {
  it('alerts every valid NEW order regardless of payment status (cash must alert too)', () => {
    expect(isAlertWorthy(order(), new Set())).toBe(true);
    expect(isAlertWorthy(order({ paymentStatus: 'pending' }), new Set())).toBe(true);
    expect(isAlertWorthy(order({ paymentStatus: 'unknown' }), new Set())).toBe(true);
    expect(isAlertWorthy(order({ status: 'Accepted' }), new Set())).toBe(false);
    expect(isAlertWorthy(order({ status: 'Draft' }), new Set())).toBe(false);
