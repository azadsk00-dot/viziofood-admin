...
export function ModalSheet(props: {
  ...
  // Keyboard-aware height: while the soft keyboard is up, shrink the sheet so
  // the pinned action footer stays visible ABOVE it (otherwise the fixed
  // height pushes SAVE/CANCEL below the resized window — unreachable).
  const [keyboardHeight, setKeyboardHeight] = React.useState(0);
  React.useEffect(() => {
    const show = Keyboard.addListener('keyboardDidShow', (e) => setKeyboardHeight(Math.round(e.endCoordinates.height)));
    const hide = Keyboard.addListener('keyboardDidHide', () => setKeyboardHeight(0));
    return () => { show.remove(); hide.remove(); };
  }, []);
  const availableHeight = Math.max(320, height - (wide ? 160 : 180) - keyboardHeight);
  return (
    <Modal visible={props.visible} transparent animationType={wide ? 'fade' : 'slide'} onRequestClose={props.onClose}>
      <View style={styles.sheetBackdrop}>
        <View
          style={[
            styles.sheet,
            // A HARD height (not maxHeight) is essential: Yoga only lets
            // flex:1 children (the scrolling body) shrink against a
            // determinate parent height. With maxHeight alone the body takes
            // its natural content height, the sheet overflows and
            // overflow:hidden clips the action footer off-screen — the
            // "no SAVE button" bug.
            // Generous bottom clearance keeps the pinned action footer
            // clear of the system gesture-navigation inset (buttons rendered
            // inside it look fine but never receive touches).
            wide
              ? { width: Math.min(760, width - 80), height: availableHeight, borderRadius: 18 }
              : { width: '100%', height: availableHeight, borderRadius: 18 },
          ]}
        >
          ...
          {/* The action footer is ABSOLUTELY pinned to the sheet bottom —
              flex sizing proved unreliable on this layout (footer silently
              vanished: the "no SAVE button" bug), and an absolutely
              positioned bar can never be pushed off-screen by content. */}
          <ScrollView style={{ flex: 1, minHeight: 0 }} contentContainerStyle={[styles.sheetBody, { paddingBottom: 132 }]} keyboardShouldPersistTaps="handled">
            {props.children}
          </ScrollView>
          {props.footer ? <View style={styles.sheetFooterPinned}>{props.footer}</View> : null}
        </View>
      </View>
    </Modal>
  );
...
const styles = StyleSheet.create({
  ...
  sheetFooterPinned: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 14, paddingBottom: 18, borderTopWidth: 1, borderTopColor: dark.border, flexDirection: 'row', gap: 12, backgroundColor: dark.surface },
});
