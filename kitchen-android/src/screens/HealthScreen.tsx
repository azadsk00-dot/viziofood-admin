...
import { probePrinter } from '../services/printerTcp';
...
export default function HealthScreen(): React.ReactElement {
  ...
  useEffect(() => {
    void checkSupabase();
    // Direct reachability probe of the LAN printers (status dot source).
    for (const printer of Object.values(usePrintStore.getState().printers)) {
      void probePrinter(printer.host, printer.port).then((online) => {
        usePrintStore.getState().setProbe(printer.id, online);
      });
    }
  }, []);
  ...
  const rows: Array<{ label: string; value: string; state: RowState }> = [
    ...
    {
      label: 'Supabase',
      value: supabaseOk === 'ok' ? 'ONLINE' : supabaseOk === 'bad' ? `ERROR — ${(syncError ?? 'unknown').slice(0, 60)}` : 'CHECKING…',
      state: supabaseOk,
    },
