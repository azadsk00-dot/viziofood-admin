...
export type CartLine = Pick<CartItem, 'productId' | 'name' | 'price' | 'quantity' | 'modifiers' | 'combo'>;
...
/**
 * Effective unit price from a base price plus modifiers. Adjustments add to
 * the base; an override REPLACES it (pizza sizes — a $0 override keeps the
 * base). Overrides are single-select in the UI; if a crafted cart ever
 * carries more than one, the LAST override wins and later adjustments still
 * apply — deterministic on both client and server, which compute
 * identically (create-checkout mirrors this exactly).
 */
export const effectiveUnitPrice = (basePrice: number, modifiers: CartModifier[]): number => {
  let unit = basePrice;
  for (const modifier of modifiers) {
    if (modifier.priceMode === 'override') {
      if (modifier.price > 0) unit = modifier.price;
    } else {
      unit += modifier.price;
    }
  }
...
/**
 * Effective unit price of a whole cart line. Combos price as base + choice
 * upgrades + inherited extras (the child's standalone price is never added);
 * ordinary products as base + modifiers.
 */
export const lineUnitPrice = (item: Pick<CartItem, 'price' | 'modifiers' | 'combo'>): number =>
  effectiveUnitPrice(
    item.price + (item.combo ? item.combo.reduce((sum, selection) => sum + selection.upgrade, 0) : 0),
    item.modifiers,
  );

const lineSubtotalCents = (item: CartLine): number => toCents(lineUnitPrice(item)) * item.quantity;
