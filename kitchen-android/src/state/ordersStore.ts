...
interface OrdersState {
  ...
  /** Order ids this device has already auto-prepared (duplicate protection). */
...
export const useOrdersStore = create<OrdersState>()(
  persist(
    (set) => ({
      ...
      markPrepared: (id) =>
        set((state) => ({
          preparedOrderIds: state.preparedOrderIds.includes(id)
            ? state.preparedOrderIds
            : [...state.preparedOrderIds.slice(-499), id],
        })),
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        preparedOrderIds: state.preparedOrderIds,
