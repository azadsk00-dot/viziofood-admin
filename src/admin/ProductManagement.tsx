...
function Editor({ item, done, close }: { item?: Product; done: () => Promise<void>; close: () => void }) {
  ...
  // Combo choice groups for THIS product — full-replace on save. Only the
  // products staff individually ticked are ever eligible.
  const [comboGroups, setComboGroups] = useState<AdminComboGroup[]>([]);
  ...
  useEffect(() => {
    ...
    void getComboGroups(item.id).then((rows) => {
      if (active) { setComboGroups(rows); setComboLoaded(true); }
    }).catch(() => { if (active) setComboLoaded(true); });
    return () => { active = false; };
  }, [item?.id]);
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      if (value.isCombo || comboGroups.length) await saveComboGroups(saved.id, value.isCombo ? comboGroups : []);
    ...
    >
      <form id="product-form" onSubmit={save} noValidate>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginTop: 6 }}>
          {(['isCombo', 'active', 'available', 'featured', 'popular', 'vegetarian', 'vegan', 'halal', 'glutenFree'] as const).map((key) => (
                  ...
                  <tr key={product.id}>
                    ...
                    <td>
                      <div className="vz-row">
                        ...
                        <span>
                          <b>{product.name}</b>{product.isCombo && <> <Badge tone="terracotta">Combo</Badge></>}
