...
import { totals, emptyCharges, aud, lineUnitPrice } from '../lib/money';
          ...
          {cart.items.map((item) => (
            <div className="cart-line" key={item.key}>
              <div className="cart-line__main">
                ...
                {item.combo?.map((selection) => (
                  <div className="cart-line__mods" key={selection.groupId}>
                    {selection.groupName}: {selection.productName}
                    {selection.upgrade > 0 ? ` (+${aud(selection.upgrade)})` : ' (included)'}
                  </div>
                ))}
              ...
              <div className="cart-line__price">
                {aud(lineUnitPrice(item) * item.quantity)}
              </div>
            </div>
          ))}
        </Card>
        ...
        <div className="vz-stack">
          ...
          <Card pad>
            ...
            <form onSubmit={handleSubmit(pay)} noValidate>
              ...
              <div className="fulfilment-toggle" role="group" aria-label="Order type">
                {(['Pickup', 'Dine-in', 'Delivery'] as Fulfilment[])
                  .filter((method) => (method === 'Pickup' || method === 'Dine-in' ? pickupOn || method === 'Dine-in' : deliveryOn))
                  .map((method) => (
                    <button
                      key={method}
                      type="button"
