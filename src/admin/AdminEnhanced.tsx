...
export function EnhancedOrders(){
  ...
  const refresh=useCallback((event?:{eventType?:string;new?:Record<string,unknown>})=>{if(event&&(String(event.new?.payment_status??'').toLowerCase()==='paid')&&(event.eventType==='INSERT'||String(event.new?.status??'')==='New')){const id=String(event.new?.id??'');if(id)notifyNewOrder(id,event.new??{},toast);void requestNotificationPermission();} void resource.reload();},[resource.reload,toast]); useOrdersRealtime(refresh);
