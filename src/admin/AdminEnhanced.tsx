    ...
    >
      ...
      <div className="vz-stack" style={{ gap: 6, marginBottom: 16 }}>
        ...
        <span className="vz-row" style={{ gap: 6 }}>
          {order.fulfilment === 'Delivery' ? <Truck size={15} /> : order.fulfilment === 'Dine-in' ? <UtensilsCrossed size={15} /> : <MapPin size={15} />}
          {order.fulfilment}
    ...
    <Card className={`order-card order-card--${order.status}`}>
      ...
      <div className="vz-row vz-row--between">
        <span className="vz-row vz-muted" style={{ gap: 6 }}>
          {order.fulfilment === 'Delivery' ? <Truck size={15} /> : order.fulfilment === 'Dine-in' ? <UtensilsCrossed size={15} /> : <MapPin size={15} />} {order.fulfilment}
        </span>
...
export function EnhancedOrders() {
  ...
  const [fulfilment, setFulfilment] = useState<'All' | 'Pickup' | 'Delivery' | 'Dine-in'>('All');
