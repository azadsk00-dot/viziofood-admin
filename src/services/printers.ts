...
const isTableMissing = (error: { code?: string; message?: string } | null | undefined) =>
  error?.code === '42P01' || error?.code === 'PGRST205' || /relation .* does not exist/i.test(error?.message ?? '');

/**
 * PostgREST errors are plain objects, not Error instances — thrown raw they
 * surface as "Could not save the printer." and hide the actual cause. Wrap
 * them in a real Error with the most likely fix spelled out.
 */
const friendlyError = (error: { code?: string; message?: string } | null | undefined, fallback: string): Error => {
  const message = error?.message ?? '';
  if (error?.code === '23505' || /duplicate key|unique constraint/i.test(message)) {
    return new Error(`A printer with this name already exists — printer names must be unique. (${message})`);
  }
  if (error?.code === '42501' || /row-level security/i.test(message)) {
    return new Error(
      `Blocked by row-level security: your account's profiles.role must be admin or staff to manage printers. (${message})`,
    );
  }
  if (isTableMissing(error)) {
    return new Error('The printers table does not exist yet — apply the 20260826 migration first.');
  }
...
export async function savePrinter(value: Partial<PrinterConfig> & { name: string }): Promise<void> {
  ...
  if (value.id) {
    ...
    if (error) throw friendlyError(error, 'Could not save the printer.');
  } else {
...
export async function deletePrinter(id: string): Promise<void> {
  ...
  if (error) throw friendlyError(error, 'Could not delete the printer.');
}
...
export async function retryPrintJob(jobId: string): Promise<void> {
  ...
  if (error) throw friendlyError(error, 'Could not retry the print job.');
}
