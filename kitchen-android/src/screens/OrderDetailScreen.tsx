...
import { jobsForOrder } from '../lib/jobViews';
...
export default function OrderDetailScreen(): React.ReactElement {
  ...
  // NEVER derive a fresh array inside the store selector: zustand v5 +
  // useSyncExternalStore treat every new reference as a store change, which
  // re-renders, which selects again… the "Maximum update depth exceeded"
  // crash when print retries churned the jobs store. Select the stable map
  // and derive with useMemo instead.
  const jobsById = usePrintStore((s) => s.jobs);
  const jobs = useMemo(
    () => jobsForOrder(jobsById, orderId),
    [jobsById, orderId],
  );
