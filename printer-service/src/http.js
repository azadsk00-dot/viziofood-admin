...
import { renderTestTicket, resolveProtocol } from './ticket.js';
 ...
 * @param {{ port: number, token?: string, protocol?: string }} options
    ...
    try {
      if (request.method === 'GET' && url.pathname === '/health') {
        return send(200, {
          ...
          protocol: options.protocol ?? 'escpos',
      ...
      if (request.method === 'POST' && url.pathname === '/test-print') {
        ...
        const printer =
          ...
          (body.host && {
            ...
            paper_width: Number(body.width) || 80,
          }) ||
          agent.printers[0];
        if (!printer) return send(404, { error: 'no printer configured on this agent' });

        // Star mC-Print models need 'star-line'; body.protocol overrides the agent default.
        const protocol = body.protocol || options.protocol || 'escpos';
        try {
          resolveProtocol(protocol);
        } catch (error) {
          return send(400, { error: error instanceof Error ? error.message : String(error) });
        }
        const ticket = renderTestTicket(protocol, printer.name, printer.paper_width ?? 80);
        await printRaw(printer.host, printer.port ?? 9100, ticket, { timeoutMs: 10_000 });
        log('info', `test print (${protocol}) sent to ${printer.name} (${printer.host}:${printer.port})`);
        return send(200, { ok: true, protocol, printer: printer.name, host: printer.host });
      }
