...
export default function ProductsScreen(): React.ReactElement {
  ...
  // Live required-field validation (same rules as save()) — drives the
  // SAVE PRODUCT enabled state and its inline hint.
  ...
  const save = async () => {
    ...
    try {
      ...
      // The core save succeeded: close the editor and refresh the list
      // immediately — the optional modifier-group sync must never hold the
      // editor hostage if its request stalls.
      setDraft(null);
      setMessage('Saved.');
      await load();
      try {
        ...
        setMessage(groupError instanceof Error ? groupError.message : 'Saved, but modifier groups could not be updated.');
      }
    } catch (e) {
      ...
      >
        {draft ? (
          <View>
            ...
            {/* Category is a chip dropdown fed by the same categories source
                as the rest of the Product section — the value stays the
                category NAME, so existing products and the database are
                unaffected. */}
            {categories.length ? (
              <SelectField
                label="Category"
              ...
              <View style={{ marginBottom: 14 }}>
                <Text style={{ color: dark.textDim, fontWeight: '700' }}>CATEGORY</Text>
                <Text style={{ color: dark.warning, marginTop: 4 }}>
                  No categories exist yet — create one under Categories, then return here.
                </Text>
              </View>
            )}
            ...
            {/* Validation + actions live INSIDE the scrollable form: the
                keyboard can never cover or intercept them (the sheet's
                keyboardShouldPersistTaps="handled" guarantees taps land even
                while typing), and they sit at the natural end of the form. */}
            {validationMessage ? (
              <Text style={{ color: dark.warning, fontWeight: '700', marginTop: 14, textAlign: 'center' }}>
                {validationMessage}
              </Text>
            ) : null}
            {error ? (
              <Text style={{ color: dark.danger, fontWeight: '700', marginTop: 10, textAlign: 'center' }}>{error}</Text>
            ) : null}
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 16, marginBottom: 28 }}>
              <Pressable
                style={[styles.footerButton, { backgroundColor: dark.surfaceAlt }]}
                disabled={busy}
              ...
              <Pressable
                ...
                style={[styles.footerButton, { backgroundColor: dark.accent, opacity: busy || validationMessage ? 0.5 : 1 }]}
                disabled={busy || Boolean(validationMessage)}
                onPress={() => void save()}
              >
                <Text style={[styles.footerButtonText, { color: dark.accentText }]}>{busy ? 'SAVING…' : 'SAVE PRODUCT'}</Text>
              </Pressable>
            </View>
          </View>
        ) : null}
