...
/** PostgREST errors are plain objects, not Errors — show their message either way. */
const toMessage = (reason: unknown, fallback: string): string => {
...
function PrinterEditor({ printer, onClose, onSaved }: { printer: Partial<PrinterConfig> & { name: string }; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(toMessage(reason, 'Could not save the printer.'));
    } finally {
      setBusy(false);
    }
  };
...
export default function PrintersPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setError(toMessage(reason, 'Could not load printers.'));
    }
  }, []);
