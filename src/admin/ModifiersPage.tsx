...
const blankOption = (groupId: string): Omit<ModifierOption, 'id'> => ({ groupId, name: '', description: '', price: 0, pricingMode: 'adjustment', active: true, displayOrder: 0 });
  ...
  return (
    ...
    >
      <form id="modifier-option-form" onSubmit={save}>
        ...
        <Field label="Pricing mode" htmlFor="mo-mode" hint="Adjustment adds to the product price; Override replaces it (use for sizes)">
          <select id="mo-mode" value={value.pricingMode} onChange={(e) => set('pricingMode', e.target.value === 'override' ? 'override' : 'adjustment')} style={{ width: '100%', padding: '8px 10px', borderRadius: 6, border: '1px solid #d3dbe6' }}>
            <option value="adjustment">Adjustment — adds to base price</option>
            <option value="override">Override — replaces base price</option>
          </select>
        </Field>
        <Field label={value.pricingMode === 'override' ? 'Price (AUD) — full price for this option' : 'Price added (AUD)'} htmlFor="mo-price" hint={value.pricingMode === 'override' ? '0 keeps the product base price' : 'Empty = free'}>
